      ...
      <div className='max-w-7xl mx-auto px-4 py-8'>
        ...
        <div className='grid grid-cols-2 sm:flex flex-wrap gap-4'>
          {allResumes.map((resume, index) => {
            const baseColor = colers[index % colers.length];
            return (
        ...
        {showCreateResume && (
          ...
          >
            <div onClick={(e) => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
              ...
              <button type='submit' className='w-full bg-green-600 hover:bg-green-700 transition-colors text-white rounded'>Create Resume</button>
              <XIcon className=' absolute top-4 right-4 text-slate-400 cursor-pointer hover:text-slate-600 transition-colors' onClick={() => { setShowCreateResume(false); setTitle('') }} />
            </div>
          </form>
        )}
          ...
          showUploadResume && (
            ...
            >
              <div onClick={(e) => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
                ...
                <button type='submit' className='w-full bg-green-600 hover:bg-green-700 transition-colors text-white rounded flex items-center justify-center gap-2' disabled={isLoading}>
                  {isLoading && <LoaderCircleIcon className='animate-spin size-4 text-white' />}
                  {isLoading ? 'Uploading...' : ' Upload Resume'}

                </button>
                <XIcon
                  className='absolute top-4 right-4 text-slate-400 cursor-pointer hover:text-slate-600 transition-colors'
                  onClick={() => {
                    setShowUploadResume(false);
                    setTitle('');
                    setResume(null);
        ...
        {editResumeId && (
          ...
          >
            <div onClick={(e) => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
              ...
              <input onChange={(e) => setTitle(e.target.value)} value={title} type="text" placeholder='Enter resume title' className='w-full px-4 py-2 mb-4 focus:border-green-600 ring-green-600' required />
              <button type='submit' className='w-full bg-green-600 hover:bg-green-700 transition-colors text-white rounded'>Update</button>
              <XIcon className=' absolute top-4 right-4 text-slate-400 cursor-pointer hover:text-slate-600 transition-colors' onClick={() => { setEditResumeId(''); setTitle('') }} />
            </div>
          </form>
        )}
